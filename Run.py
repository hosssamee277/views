try:
    ...
    import pytesseract
    pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'  # Adjust if path differs
...
[bold white]Views :[bold green] +{self.views_sent}""", width=56, style="bold bright_white", title="[bold bright_white][ Sukses ]"))
                ...
                list(map(lambda _: (self.DELAY(0, 5), self.ANTI_LOGOUT()), range(8)))
...
[bold white]  \/_____/   \/_____/   \/_/     \/_____/   \/_____/
        [underline green]Free Tiktok Views - Coded by Hossam""", width=56, style="bold bright_white")
        )
        return (True)
